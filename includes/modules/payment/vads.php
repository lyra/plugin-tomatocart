...
/**
 * Front office code for vads payment method (payment buttons, return management...)
 *
 */
class osC_Payment_vads extends osC_Payment {
	/**
	...
	var $_sort_order, $_order_id, $_transaction_response;

	/**
	 * Constructor : set some tomato variables (status, title...)
	 */
	function osC_Payment_vads() {
		...
		// "Redirect to payment page" kind of method => define url
		$this->form_action_url = defined('MODULE_PAYMENT_VADS_PLATFORM_URL')
				? MODULE_PAYMENT_VADS_PLATFORM_URL
				: 'https://secure.payzen.eu/vads-payment/';

		// Check zone if necessary
		if ($this->_status === true) {
			$this->order_status = (int) MODULE_PAYMENT_VADS_ORDER_STATUS_ID > 0
					? (int) MODULE_PAYMENT_VADS_ORDER_STATUS_ID
					: (int) ORDERS_STATUS_PAID;

			if ((int) MODULE_PAYMENT_VADS_ZONE > 0) {
				$check_flag = false;

				$Qcheck = $osC_Database->query(
								'select zone_id from :table_zones_to_geo_zones where geo_zone_id = :geo_zone_id and zone_country_id = :zone_country_id order by zone_id');
				$Qcheck->bindTable(':table_zones_to_geo_zones',
								TABLE_ZONES_TO_GEO_ZONES);
				$Qcheck->bindInt(':geo_zone_id', MODULE_PAYMENT_PAYPAL_IPN_ZONE);
				$Qcheck->bindInt(':zone_country_id',
								$osC_ShoppingCart->getBillingAddress('country_id'));
				$Qcheck->execute();
				...
				while ($Qcheck->next()) {
					if ($Qcheck->valueInt('zone_id') < 1) {
						$check_flag = true;
						break;
					} elseif ($Qcheck->valueInt('zone_id')
							== $osC_ShoppingCart->getBillingAddress('zone_id')) {
						$check_flag = true;
						break;
					}
				}
	...
	/**
	 * Check if there is any error in payment data before sending client to gateway
	 */
	function pre_confirmation_check() {
		global $osC_Customer, $osC_Currencies, $osC_ShoppingCart, $osC_Tax, $osC_Language, $messageStack;
		$messageStack->reset();
		$vads = $this->_getLoadedVadsApi();
		if($vads->get('ctx_mode')=='PRODUCTION') {
			$vads->clearInvalidOptionnalFields();
		}
		if (!$vads->isRequestReady($errors)) {
			foreach ($errors as $error) {
				$messageStack->add_session('checkout_payment',
								$osC_Language->get('module_payment_vads_form_error') . ' '
										. $error, 'error');
			}
		}
	}
	...
	/**
	 * Save order in "preparing" status when redirecting client to gateway
	 */
	...
	function process_button() {
		...
		foreach ($vads->getRequestFields() as $field) {
			$process_button_string .= osc_draw_hidden_field($field->getName(),
					$field->getValue());
		}
	...
	/**
	 * Manage check url call
	 */
	function callback() {
		global $osC_Database;

		$vads = new VadsApi();
		$resp = $vads->getResponse($_REQUEST, MODULE_PAYMENT_VADS_CTX_MODE,
		...
		$comment = $resp->message . ' (' . $resp->code . ')';

		// Get useful parameters
		$order_id = $resp->get('order_id');
		$already_paid = osC_Order::getStatusID($order_id)
				!= ORDERS_STATUS_PREPARING;

		if (!osC_Order::exists($order_id)) {
		...
		if ($resp->isAcceptedPayment()) {
			if (!$already_paid) {
				...
				//				osC_Order::process($order_id, ORDERS_STATUS_PREPARING, $comment);
				echo $resp->getOutputForGateway('payment_ok_already_done');
			}
		} else {
			if (!$already_paid) {
				osC_Order::remove($order_id);
	...
	/**
	 * (non-PHPdoc)
	 * @see osC_Payment::process()
	 */
	function process() {
		// Manage client return from gateway
		...
		// Get useful parameters
		$order_id = $resp->get('order_id');
		$already_paid = osC_Order::exists($order_id)
				&& osC_Order::getStatusID($order_id) != ORDERS_STATUS_PREPARING;

		// Check payment success
		if ($resp->isAcceptedPayment()) {
			if (!$already_paid) {
				osC_Order::process($order_id, $this->order_status, $comment);
				$messageStack->add_session('checkout',
								$osC_Language->get('module_payment_vads_check_url_warning'),
								'warning');
			...
			if (!$already_paid) {
				osC_Order::remove($order_id);
				$messageStack->add_session('checkout', $resp->message, 'error');
				osc_redirect(osc_href_link(FILENAME_CHECKOUT, 'checkout', 'SSL'));
			} else {
				$comment = $osC_Language->get(
								'failure_notification_on_registered_order') . ' '
						. $resp->message;
				$messageStack->add_session('checkout', $comment, 'error');
				osc_redirect(osc_href_link(FILENAME_CHECKOUT, 'checkout', 'SSL'));
			}
		}
	}
	...
	/**
	 * Return a VadsApi object loaded with admin configuration and runtime parameters
	 * @param boolean $for_request true if we are sending the client to the gateway, false if we are managing the gateway response
	 * @return VadsApi $vads
	 */
	function _getLoadedVadsApi($for_request = true) {
		global $osC_Customer, $osC_Currencies, $osC_ShoppingCart, $osC_Tax, $osC_Language, $messageStack;
		...
		foreach ($config_names as $vads_name) {
			$tomato_name = strtoupper('MODULE_PAYMENT_' . $vads_name);
			if (defined($tomato_name)) {
				$vads->set($vads_name, constant($tomato_name));
			}
		}
		...
		if ($for_request) {
			...
			// Get language
			$language = substr($osC_Language->getCode(), 0, 2);
			if (!in_array($language, $vads->getSupportedLanguages())) {
				$language = defined('MODULE_PAYMENT_VADS_LANGUAGE')
						? MODULE_PAYMENT_VADS_LANGUAGE
						: 'fr';
			}

			// Currency conversion
			$change_rate = $osC_Currencies->value($osC_Currencies->getCode());
			$decimal_mult = pow(10, $osC_Currencies->getDecimalPlaces());

			$params = array(
					'vads_amount' => round(
							$decimal_mult * $change_rate * $osC_ShoppingCart->getTotal()),
					'vads_contrib' => 'TomatoCart1.1.3_1.2a',
					'vads_currency' => $vads->getCurrencyNumCode(
									$osC_Currencies->getCode()),
					'vads_cust_address' => $osC_ShoppingCart->getBillingAddress(
									'street_address'),
					'vads_cust_country' => $osC_ShoppingCart->getBillingAddress(
									'country_iso_code_2'),
					'vads_cust_email' => $osC_Customer->getEmailAddress(),
					'vads_cust_id' => $osC_Customer->getID(),
					'vads_cust_name' => $osC_ShoppingCart->getBillingAddress('firstname')
							. ' ' . $osC_ShoppingCart->getShippingAddress('lastname'),
					'vads_cust_phone' => $osC_ShoppingCart->getBillingAddress('telephone'),
					'vads_cust_title' => $osC_Language->get(
									'payment_vads_gender_' . $osC_Customer->getGender()),
					'vads_cust_city' => $osC_ShoppingCart->getBillingAddress('city'),
					'vads_cust_zip' => $osC_ShoppingCart->getBillingAddress('postcode'),
					'vads_language' => substr($osC_Language->getCode(), 0, 2),
					'vads_order_id' => $this->_order_id);

			$vads->setFromArray($params);
		}

		$this->vads_api = $vads;
		return $this->vads_api;
	}
}
